...
// special defines for lslidarlidar support
    ...
    public:
        ...
        // 比较函数，用于排序角度区间
        static bool compareRanges(const LslidarDriver::AngleRange& a, const LslidarDriver::AngleRange& b) 
            { return a.start < b.start;}
        // std::vector<LslidarDriver::AngleRange> usableAngles{};
        std::vector<LslidarDriver::AngleRange> disableAngles{};
        std::vector<LslidarDriver::AngleRange> mergeRanges(std::vector<LslidarDriver::AngleRange>& ranges);
        std::vector<LslidarDriver::AngleRange> calculateDisableAngles(const std::vector<long int>& disableMin, const std::vector<long int>& disableMax);
        bool is_skip_angle(size_t fir_idx);    
