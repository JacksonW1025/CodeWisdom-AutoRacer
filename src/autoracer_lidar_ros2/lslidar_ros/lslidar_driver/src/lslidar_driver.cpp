...
namespace lslidar_driver {
    ...
    bool LslidarDriver::loadParameters() {
        ...
        this->declare_parameter<std::vector<int>>("angle_disable_min",{0});
        this->declare_parameter<std::vector<int>>("angle_disable_max",{0});
    ...
    bool LslidarDriver::createRosIO() {
        ...
        scan_pub = this->create_publisher<sensor_msgs::msg::LaserScan>("/scan_raw", 10);
    ...
    // 合并重叠的角度区间
    std::vector<LslidarDriver::AngleRange> LslidarDriver::mergeRanges(std::vector<LslidarDriver::AngleRange>& ranges) {
        std::vector<LslidarDriver::AngleRange> mergedRanges;
        if (ranges.empty()) return mergedRanges;
    ...
    // 计算非禁用的角度区间
    std::vector<LslidarDriver::AngleRange> LslidarDriver::calculateDisableAngles(const std::vector<long int>& disableMin, const std::vector<long int>& disableMax) {
        std::vector<LslidarDriver::AngleRange> allRanges;
        ...
        // 合并所有禁用区间
        std::vector<LslidarDriver::AngleRange> mergedDisableRanges = mergeRanges(allRanges);
    
        return mergedDisableRanges;
    }
    ...
    bool LslidarDriver::is_skip_angle(size_t fir_idx){
        for (auto& range : disableAngles) {
            if((firings.azimuth[fir_idx] >= range.start ) && firings.azimuth[fir_idx] < range.end) return false;
        }
        return true;
    }
    ...
    bool LslidarDriver::poll() {
        ...
        for (size_t fir_idx = start_fir_idx; fir_idx < end_fir_idx; ++fir_idx) {
            ...
            if (!(disableAngles[0].start==0&&disableAngles[0].end==0))
                if (!is_skip_angle(fir_idx)) continue;
